...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message); // Trigger the error toast
      }
      ...
      toast.promise(
        ...
        }),
        {
          success: {
            render() {
              // Navigate to the dashboard with the email parameter
...
const RegisterForm = () => {
  const handleRegister = async (e) => {
    ...
    const promise = fetch("https://schoolcafe.ng/api/users.php?action=register", {
      ...
      body: JSON.stringify({ username, email, referral_code: referralCode, password }),
    });
